...
If `console-ninja` is not available in an external terminal session after executing `Console Ninja: Start` command from VS-Code command pallete and reloading your terminal session then you may need to add `~/.console-ninja/.bin` to `PATH` manually. On Windows, depending on how you start apps, you may need to restart Explorer process or re-login to the system.
